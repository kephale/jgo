...
class ExecutableNotFound(Exception):
    def __init__(self, executable, path):
        super(ExecutableNotFound,
              self).__init__('{} not found on path {}'.format(
                  executable, path))


class InvalidEndpoint(Exception):
    def __init__(self, endpoint, reason):
        super(InvalidEndpoint, self).__init__('Invalid endpoint {}: {}'.format(
            endpoint, reason))
...
class UnableToAutoComplete(Exception):
    def __init__(self, clazz):
        super(UnableToAutoComplete,
              self).__init__('Unable to auto-complete {}'.format(clazz))
        self.clazz = clazz
...
class Endpoint():
    ...
    def jar_name(self):
        return '-'.join(([self.artifactId, self.classifier] if self.classifier
                         else [self.artifactId]) + [self.version]) + '.jar'
    ...
    def is_endpoint(string):
        endpoint_elements = string.split('+')[0].split(
            ':') if '+' in string else string.split(':')

        if len(endpoint_elements) < 2 or len(
                endpoint_elements) > 5 or endpoint_elements[0].startswith('-'):
            return False
    ...
    def parse_endpoint(endpoint):
        ...
        if (endpoint_elements_count == 4):
            return Endpoint(*endpoint_elements[:3],
                            main_class=endpoint_elements[3])
        ...
        if (endpoint_elements_count == 3):
            if re.match(
                    ...
                    endpoint_elements[2]):
                return Endpoint(*endpoint_elements[:2],
                                version=endpoint_elements[2])
            else:
                return Endpoint(*endpoint_elements[:2],
                                main_class=endpoint_elements[2])
...
def link(source, link_name, link_type='auto'):
    _logger.debug("Linking source %s to target %s with link_type %s", source,
                  link_name, link_type)
    ...
    raise Exception('Unable to link source {} to target {} with link_type {}',
                    source, link_name, link_type)
...
def launch_java(
    ...
    **subprocess_run_kwargs,
    ...
    cp = classpath_separator().join([os.path.join(jar_dir, '*')] +
                                    additional_jars)
...
def find_endpoint(argv, shortcuts={}):
    ...
    for index, arg in enumerate(argv):
        if arg in shortcuts or (Endpoint.is_endpoint(arg)
                                and not pattern.match(arg)):
            indices.append(index)
    return -1 if len(indices) == 0 else indices[-1]


_default_log_levels = ('NOTSET', 'DEBUG', 'INFO', 'WARN', 'ERROR', 'CRITICAL',
                       'FATAL', 'TRACE')
...
'''
    ...
    parser = argparse.ArgumentParser(
        ...
        usage=
        '%(prog)s [-v] [-u] [-U] [-m] [-q] [--log-level] [--ignore-jgorc] [--link-type type] [--additional-jars jar [jar ...]] [--additional-endpoints endpoint [endpoint ...]] [JVM_OPTIONS [JVM_OPTIONS ...]] <endpoint> [main-args]',
        epilog=epilog,
    ...
    parser.add_argument('--additional-endpoints',
                        nargs='+',
                        help='Add additional endpoints',
                        default=[],
                        required=False)
    parser.add_argument('--ignore-jgorc',
                        action='store_true',
                        help='Ignore ~/.jgorc')
    parser.add_argument(
        '--link-type',
        ...
        type=str,
        help=
        'How to link from local maven repository into jgo cache. Defaults to the `links\' setting in ~/.jgorc or \'auto\' if not specified.',
...
def default_config():
    ...
    config.set('settings', 'm2Repo',
               os.path.join(str(pathlib.Path.home()), '.m2', 'repository'))
    config.set('settings', 'cacheDir',
               os.path.join(str(pathlib.Path.home()), '.jgo'))
...
def expand_coordinate(coordinate, shortcuts={}):
    ...
    while was_changed:
        ...
        for shortcut, replacement in shortcuts.items():
            if shortcut not in used_shortcuts and coordinate.startswith(
                    shortcut):
                _logger.debug("Replacing %s with %s in %s.", shortcut,
...
def autocomplete_main_class(main_class, artifactId, workspace):
    ...
    if main_class[0] == '@':
        ...
        relevant_jars = [
            jar for jar in glob.glob(os.path.join(workspace, '*.jar'))
...
def workspace_dir_from_coordinates(coordinates, cache_dir):
    ...
    coord_string = ['-'.join(c) for c in coordinates[1:]]
    hash_path = hashlib.sha256(coord_string.encode('utf-8')).hexdigest()
    workspace = '+'.join([workspace, hash_path])
    return workspace


def workspace_dir_from_endpoint_strings(endpoint_strings,
                                        cache_dir,
                                        shortcuts={}):
...
def resolve_dependencies(endpoint_string,
                         cache_dir,
                         m2_repo,
                         link_type='auto',
                         update_cache=False,
    ...
    deps = ''.join('<dependency>{}</dependency>'.format(ep.dependency_string())
                   for ep in endpoints)
    repo_str = ''.join(
        '<repository><id>{rid}</id><url>{url}</url></repository>'.format(rid=k,
                                                                         url=v)
        for (k, v) in repositories.items())
    coordinates = coordinates_from_endpoints(endpoints)
    workspace = workspace_dir_from_coordinates(coordinates,
                                               cache_dir=cache_dir)
...
'''.format(groupId=primary_endpoint.groupId,
    ...
    for l in str(mvn_out).split('\\n'):
        ...
        if re.match('.*:(compile|runtime)', l) and not re.match(
                '.*\\[DEBUG\\]', l) and not re.match('.*:provided', l):
            ...
            try:
                link(os.path.join(m2_repo, *g.split('.'), a, version,
                                  jar_file),
...
def run(parser, argv=sys.argv[1:], stdout=None, stderr=None):
    ...
    if endpoint_index == -1:
        raise HelpRequested(
            argv) if '-h' in argv or '--help' in argv else NoEndpointProvided(
                argv)
    ...
    endpoint_string = '+'.join([argv[endpoint_index]] +
                               args.additional_endpoints)
    ...
    main_class_file = os.path.join(
        workspace, primary_endpoint.main_class
    ) if primary_endpoint.main_class else os.path.join(workspace, 'mainClass')
    try:
        with open(main_class_file, 'r') as f:
            main_class = f.readline()
        return launch_java(workspace,
                           jvm_args,
                           main_class,
                           *program_args,
    ...
    if not primary_endpoint.main_class:
        ...
        jar_path = glob.glob(
            os.path.join(workspace, primary_endpoint.jar_name()).replace(
                Endpoint.VERSION_RELEASE, '*').replace(Endpoint.VERSION_LATEST,
                                                       '*'))[0]
    ...
    main_class = autocomplete_main_class(main_class,
                                         primary_endpoint.artifactId,
                                         workspace)
